import { Token } from '@modules/auth/index/domain/entities';
import { TokenActionEnum } from '@modules/auth/index/domain/enums';
import { InvalidConfirmationTokenException } from '@modules/auth/index/domain/exceptions';
import { TokenBlackListedException } from '@modules/auth/index/domain/exceptions/token-black-listed.exception';
...
import { JwtConfigInterface } from '@src/config';
...
export class TokenService
{
    ...
    {
        const { iss, aud, refreshExpires } = this.configService.get<JwtConfigInterface>('jwt');

        const basePayload: DecodeTokenInterface = {
            iss,
            aud,
            sub: user.email,
            userId: user._id,
        ...
        const jWTToken = new JwtModel(
            user,
            this.jwtService.decode(hash) as DecodeTokenInterface,
            this.jwtService.decode(refreshHash) as DecodeTokenInterface,
            hash,
            refreshHash);
    ...
    decodeToken(token: string): DecodeTokenInterface
    {
        return this.jwtService.decode(token) as DecodeTokenInterface;
    }
    ...
    {
        dayjs.extend(utc);
        const { iss, aud, confirmationExpires } = this.configService.get<JwtConfigInterface>('jwt');
    ...
    async verifyToken(token: string): Promise<DecodeTokenInterface>
    {
        try
        {
